...
def train(config, workdir):
    ...
    """
    fabric = Fabric(accelerator=config.device.type, precision="bf16-true")
    fabric.launch()
    # Création des dossiers pour les échantillons et TensorBoard.
    ...
    if sde_name == 'vpsde':
        ...
        sde = sde_lib.OUVESDE(sigma_min=config.model.sigma_min,
                              sigma_max=config.model.sigma_max,
                              N=config.model.num_scales)
        sampling_eps = 1e-5
    else:
    ...
    for step in range(initial_step, num_train_steps + 1):
        ...
        if step % config.training.eval_freq == 0:
            ...
            if eval_img_perfect.ndim == 4 and eval_img_perfect.shape[-1] != config.data.num_channels:
        ...
        if (step != 0 and step % config.training.snapshot_freq == 0) or (step == num_train_steps):
            ...
            if config.training.snapshot_sampling:
                ...
                sampling_fn = sampling.get_sampling_fn(config, sde, sampling_shape, inverse_scaler, sampling_eps, y=real_rir)
                sample, n = sampling_fn(score_model)
                ema.restore(score_model.parameters())

                # Sélection du premier exemple du batch pour la comparaison
                perfect_rir_sample = perfect_rir[0].detach().cpu().numpy()     # forme: (epaisseur=1, longueur, canaux)
                generated_sample = sample[0].detach().cpu().numpy()            # forme: (epaisseur=1, longueur, canaux)
                # Suppression de la dimension "épaisseur" (qui vaut 1)
                perfect_rir_sample = np.squeeze(perfect_rir_sample, axis=0)  # devient (longueur, canaux)
                generated_sample = np.squeeze(generated_sample, axis=0)      # devient (longueur, canaux)

                # On trace jusqu'à 32 canaux (ou le nombre maximum de canaux disponibles)
                plt.ioff()
                num_channels = min(32, perfect_rir_sample.shape[1])
                fig, axes = plt.subplots(nrows=num_channels, ncols=1, sharex=True, figsize=(6, 12))
...
def evaluate(config, workdir, eval_folder="eval"):
    ...
    _, eval_loader, _ = datasets.get_dataset(config,
                                             uniform_dequantization=config.data.uniform_dequantization,
                                             evaluation=True)
    ...
    if sde_name == 'vpsde':
        sde = sde_lib.VPSDE(beta_min=config.model.beta_min,
                           ...
                           N=config.model.num_scales)
    if sde_name == 'ouvesde':
        sde = sde_lib.OUVESDE(beta_min=config.model.beta_min,
                           beta_max=config.model.beta_max,
                           N=config.model.num_scales)
        sampling_eps = 1e-3
    elif sde_name == 'subvpsde':
    ...
    train_loader_bpd, eval_loader_bpd, _ = datasets.get_dataset(config,
    ...
    for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
        ...
        try:
            ...
            logging.warning("Problème lors du chargement du checkpoint, nouvelle tentative dans 60s...")
            time.sleep(60)
            try:
                state = restore_checkpoint(ckpt_filename, state, device=config.device)
            except Exception as e:
        ...
        if config.eval.enable_loss:
            ...
            for i, batch in enumerate(eval_loader):
                ...
                if (i + 1) % 1000 == 0:
                    logging.info("Évaluation loss, étape %d sur %d", i + 1, len(eval_loader))
            all_losses = np.asarray(all_losses)
            loss_filepath = os.path.join(eval_dir, f"ckpt_{ckpt}_loss.npz")
            with open(loss_filepath, "wb") as f:
                np.savez_compressed(f, all_losses=all_losses, mean_loss=all_losses.mean())
        ...
        if config.eval.enable_bpd:
            ...
            for repeat in range(bpd_num_repeats):
                ...
                for batch_id, batch in enumerate(bpd_iter):
                    ...
                    bpds.extend(bpd_val)
                    logging.info("ckpt: %d, repeat: %d, batch: %d, moyenne bpd: %.6f",
                                 ckpt, repeat, batch_id, np.mean(np.asarray(bpds)))
                    bpd_round_id = batch_id + len(ds_bpd) * repeat
                    bpd_filepath = os.path.join(eval_dir,
        ...
        if config.eval.enable_sampling:
            ...
            for r in range(num_sampling_rounds):
                ...
                latents = evaluation.run_inception_distributed(samples_np, inception_model,
                                                               inceptionv3=inceptionv3)
                gc.collect()
                stats_filepath = os.path.join(this_sample_dir, f"statistics_{r}.npz")
                with open(stats_filepath, "wb") as f:
                    np.savez_compressed(f, pool_3=latents["pool_3"], logits=latents["logits"])
